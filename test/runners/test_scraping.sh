#!/bin/bash
# Test runner for Scraping tests (18 files - excludes 2 integration tests)
# Usage: ./test/runners/test_scraping.sh
# Note: Automatically skips tests tagged with @moduletag :integration
...
for i in "${!files[@]}"; do
  ...
  # Check if file contains integration tags (these hit live APIs)
  if grep -q "@moduletag :integration" "$file" 2>/dev/null; then
    echo "⚠ SKIPPED (integration test - hits live APIs)"
...
echo "SKIPPED: $skipped_count/18 (integration tests)"
