defmodule EhsEnforcementWeb.DashboardTestHelpers do
  ...
  def create_dashboard_test_data do
    ...
    cases =
      create_test_cases([
        # Recent HSE cases
        {hse, Enum.at(offenders, 0), Date.add(base_date, 25), "15000.00", "HSE-2024-001",
         "Failure to ensure workplace safety"},
        {hse, Enum.at(offenders, 1), Date.add(base_date, 22), "8500.00", "HSE-2024-002",
         "Inadequate risk assessment"},
        {hse, Enum.at(offenders, 0), Date.add(base_date, 20), "22000.00", "HSE-2024-003",
         "Multiple safety violations"},
        {hse, Enum.at(offenders, 2), Date.add(base_date, 15), "12000.00", "HSE-2024-004",
         "Equipment safety breach"},

        # Environment Agency cases
        {ea, Enum.at(offenders, 2), Date.add(base_date, 24), "12000.00", "EA-2024-001",
         "Illegal waste disposal"},
        {ea, Enum.at(offenders, 3), Date.add(base_date, 18), "35000.00", "EA-2024-002",
         "Water pollution incident"},
        {ea, Enum.at(offenders, 4), Date.add(base_date, 12), "8000.00", "EA-2024-003",
         "Air quality violation"},

        # Older cases for timeline diversity
        {hse, Enum.at(offenders, 3), Date.add(base_date, -15), "5000.00", "HSE-2023-099",
         "Historic safety breach"},
        {ea, Enum.at(offenders, 1), Date.add(base_date, -10), "7500.00", "EA-2023-087",
         "Previous environmental breach"}
      ])
    ...
    %{
      ...
      ea: ea,
      onr: onr,
      # Most recent for timeline
      recent_cases: Enum.take(cases, 10)
    }
  end
  ...
  def create_performance_test_data(opts \\ []) do
    ...
    # Create offenders
    offenders =
      Enum.map(1..offender_count, fn i ->
        {:ok, offender} =
          Enforcement.create_offender(%{
  ...
  def simulate_sync_operation(view, agency_code, opts \\ []) do
    # milliseconds
    duration = Keyword.get(opts, :duration, 100)
  ...
  def assert_sync_status(view, agency_name, expected_status) do
    ...
    case expected_status do
      ...
      :in_progress ->
        assert status_content =~ "Syncing" or status_content =~ "In Progress" or
                 status_content =~ "%"

      :complete ->
        assert status_content =~ "Complete" or status_content =~ "Success" or
                 status_content =~ "ago"
  ...
  """
  def assert_dashboard_summary(
        view,
        expected_total_cases,
        expected_total_agencies,
        expected_total_fines \\ nil
      ) do
    html = render(view)
  ...
  def assert_performance(
        operation,
        max_time_ms,
        message \\ "Operation should complete within time limit"
      ) do
  ...
  def create_edge_case_data do
    ...
    {:ok, long_name_agency} =
      Enforcement.create_agency(%{
        code: :long,
        name:
          "This is an Extremely Long Agency Name That Might Cause Layout Issues in the User Interface",
        enabled: true
      })
