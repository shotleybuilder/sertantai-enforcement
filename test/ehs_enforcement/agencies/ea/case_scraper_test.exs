defmodule EhsEnforcement.Agencies.Ea.CaseScraperTest do
  ...
  describe "parse_summary_page/2" do
    test "parses EA summary page HTML correctly" do
      ...
      result =
        call_private(CaseScraper, :parse_summary_page, [@mock_ea_summary_html, action_type])
      ...
      # Empty address should be nil
      assert first_record.summary_address == nil
      # DD/MM/YYYY -> YYYY-MM-DD
      assert first_record.action_date == ~D[2024-01-11]
  ...
  describe "date parsing" do
    test "parses DD/MM/YYYY format correctly" do
      ...
      # Leap year
      date_result = call_private(CaseScraper, :parse_ea_date, ["29/02/2024"])
  ...
  describe "record ID extraction" do
    test "extracts record ID from EA URLs correctly" do
      url1 =
        "/public-register/enforcement-action/registration/10002155?__pageState=result-enforcement-action"

      result1 = call_private(CaseScraper, :extract_record_id_from_url, [url1])
      assert result1 == "10002155"

      url2 =
        "https://environment.data.gov.uk/public-register/enforcement-action/registration/10001620"
  ...
  describe "URL building" do
    test "builds absolute detail URLs correctly" do
      ...
      expected =
        "https://environment.data.gov.uk/public-register/enforcement-action/registration/10002155?__pageState=result-enforcement-action"
    ...
    test "handles already absolute URLs" do
      absolute_url =
        "https://environment.data.gov.uk/public-register/enforcement-action/registration/10002155"
  ...
  describe "row extraction validation" do
    test "validates all required fields are present" do
      ...
      result =
        call_private(CaseScraper, :extract_summary_record_from_row, [
          first_row,
          :court_case,
          DateTime.utc_now()
        ])
