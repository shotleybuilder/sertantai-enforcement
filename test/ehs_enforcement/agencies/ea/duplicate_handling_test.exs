defmodule EhsEnforcement.Agencies.Ea.DuplicateHandlingTest do
  ...
  describe "EA duplicate regulator_id handling" do
    ...
    defp build_ea_record(regulator_id, company_name \\ "Test Company", fine_amount \\ 1000) do
      %{
        ea_record_id: "test_#{:rand.uniform(999999)}",
        offender_name: company_name,
        address: "123 Test Street", 
        action_date: ~D[2024-01-30],
        action_type: :court_case,
        company_registration_number: "TEST#{:rand.uniform(999999)}",
        ...
        event_reference: "EVT_#{:rand.uniform(999999)}", 
    ...
    test "handles duplicate regulator_id with no changes by returning existing case (no update)", %{admin_user: admin_user} do
      ...
      assert second_result.id == first_case.id  # Same record
      ...
      case DateTime.compare(second_result.updated_at, original_updated_at) do
        ...
        diff ->
          ...
          flunk("updated_at should be unchanged when no fields change. Original: #{original_updated_at}, Second: #{second_result.updated_at}")
      end
    end
    
    test "scraping session counts duplicates as 'existing' not 'created' for UI display", %{admin_user: admin_user} do
      ...
      {:ok, _first_case, :created} = CaseProcessor.process_and_create_case_with_status(transformed_data, admin_user)
      
      # Test that duplicate returns proper status for UI display
      duplicate_result = CaseProcessor.process_and_create_case_with_status(transformed_data, admin_user)
    ...
    test "handles duplicate regulator_id by updating existing case when data actually changed", %{admin_user: admin_user} do
      ...
      case result do
        ...
        {:error, %Ash.Error.Invalid{errors: errors}} ->
          ...
          duplicate_error = Enum.find(errors, fn error ->
            error.field == :regulator_id and 
            String.contains?(error.message, "already been taken")
          end)
          
          if duplicate_error do
            flunk("BUG REPRODUCED: Duplicate regulator_id causes constraint violation instead of update. Error: #{duplicate_error.message}")
          else
    ...
    test "handles multiple different duplicate regulator_ids in same session", %{admin_user: admin_user} do
      ...
      original_cases = for {regulator_id, company_name} <- duplicates do
      ...
      for {{regulator_id, company_name}, _original_case} <- Enum.zip(duplicates, original_cases) do
        ...
        case result do
          ...
          {:error, %Ash.Error.Invalid{errors: errors}} ->
            ...
            constraint_error = Enum.find(errors, fn error ->
              error.field == :regulator_id and 
              String.contains?(error.message, "already been taken")
            end)
            
            if constraint_error do
              flunk("BUG: regulator_id #{regulator_id} caused constraint violation: #{constraint_error.message}")
            else
