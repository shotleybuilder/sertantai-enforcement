defmodule EhsEnforcement.Scraping.Ea.IntegrationTest do
  ...
  describe "EA scraping integration" do
    ...
    test "EA CaseScraper functions are called with correct parameters", %{admin_user: admin_user} do
      ...
      assert {:ok, summary_records} = CaseScraper.collect_summary_records_for_action_type(
        date_from, 
        date_to, 
        action_type, 
        opts
    ...
    test "EA individual processing functions exist and have correct arity", %{admin_user: admin_user} do
      ...
      case CaseScraper.fetch_detail_record_individual(summary_record, opts) do
        ...
        {:error, _reason} -> assert true  # Expected - no real URL
      end
    end
    ...
    test "EA DataTransformer handles nil values without crashing", %{admin_user: admin_user} do
      ...
      ea_record_with_nils = %{
        ea_record_id: "test123",
        offender_name: nil,  # This should not crash clean_company_name/1
        address: nil,        # This should not crash normalize_address/1
    ...
    test "EA Case creation now works with CaseProcessor fix", %{admin_user: admin_user} do
      ...
      ea_record = %{
        ...
        section: "Section 33",
        scraped_at: DateTime.utc_now(),
        detail_url: "https://environment.data.gov.uk/public-register/enforcement-action/registration/10000529"
      }
      ...
      case CaseProcessor.process_and_create_case(transformed_case_data, admin_user) do
        ...
        {:error, %Ash.Error.Invalid{errors: errors}} ->
          ...
          error_messages = Enum.map(errors, fn error ->
            case error do
