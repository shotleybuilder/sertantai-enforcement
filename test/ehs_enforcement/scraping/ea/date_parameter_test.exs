defmodule EhsEnforcement.Scraping.Ea.DateParameterTest do
  ...
  describe "EA date parameter flow" do
    ...
    test "EA scraping parameters are correctly extracted from form", %{admin_user: admin_user} do
      ...
      Logger.info("Original scrape_request dates: from=#{inspect(scrape_request.date_from)}, to=#{inspect(scrape_request.date_to)}")
      Logger.info("Extracted ea_params dates: from=#{inspect(ea_params.date_from)}, to=#{inspect(ea_params.date_to)}")
    ...
    test "EA agency validation preserves dates", %{admin_user: admin_user} do
      ...
      Logger.info("🧪 Testing EA.validate_params with dates: from=#{Keyword.get(ea_validation_opts, :date_from)}, to=#{Keyword.get(ea_validation_opts, :date_to)}")
      
      case Ea.validate_params(ea_validation_opts) do
        {:ok, validated_params} ->
          Logger.info("✅ EA Validation successful")
          Logger.info("Validated dates: from=#{validated_params.date_from}, to=#{validated_params.date_to}")
