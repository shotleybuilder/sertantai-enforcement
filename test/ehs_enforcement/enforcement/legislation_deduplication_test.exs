defmodule EhsEnforcement.Enforcement.LegislationDeduplicationTest do
  ...
  describe "find_or_create_legislation/4" do
    test "creates new legislation when none exists" do
      ...
      assert {:ok, legislation} = Enforcement.find_or_create_legislation(title, year, number, type)
    ...
    test "returns existing legislation for exact match" do
      ...
      assert {:ok, found} = Enforcement.find_or_create_legislation(
    ...
    test "normalizes title before processing" do
      ...
      assert {:ok, legislation} = Enforcement.find_or_create_legislation(title, 2002, 2677, :regulation)
      
      assert legislation.legislation_title == "Control of Substances Hazardous to Health Regulations"
    end
  ...
  describe "batch_find_or_create_legislation/1" do
    ...
    test "handles mixed success and failure" do
      legislation_data = [
        ...
        %{title: "", year: 2021, number: 2, type: :regulation},  # Invalid
        %{title: "Another Valid Act", year: 2022, number: 3, type: :act}
      ]
      
      assert {:error, {"", _reason}} = Enforcement.batch_find_or_create_legislation(legislation_data)
    end
  end
  ...
  describe "search_legislation_fuzzy/2" do
    setup do
      ...
      {:ok, _} = Enforcement.create_legislation(%{
        legislation_title: "Health and Safety at Work etc. Act",
        legislation_year: 1974,
        legislation_number: 37,
        legislation_type: :act
    ...
    test "finds exact matches" do
      assert {:ok, matches} = Enforcement.search_legislation_fuzzy("Health and Safety at Work etc. Act")
    ...
    test "finds similar matches with fuzzy search" do
      assert {:ok, matches} = Enforcement.search_legislation_fuzzy("HEALTH AND SAFETY AT WORK ACT")
    ...
    test "respects similarity threshold" do
      ...
      assert {:ok, matches_low} = Enforcement.search_legislation_fuzzy("Completely Different Act", 0.9)
      assert {:ok, matches_high} = Enforcement.search_legislation_fuzzy("Completely Different Act", 0.1)
      
      assert length(matches_low) <= length(matches_high)
    end
    
    test "returns empty list for no matches" do
      assert {:ok, matches} = Enforcement.search_legislation_fuzzy("Nonexistent Legislation XYZ", 0.8)
  ...
  describe "get_legislation_stats/0" do
    ...
    test "detects potential duplicates" do
      ...
      {:ok, _} = Enforcement.create_legislation(%{
        legislation_title: "COMPLETE ACT",  # Same as "Complete Act" but different case
        legislation_year: 2020,
        legislation_number: 1,
        legislation_type: :act
      })
      ...
      complete_act_duplicate = Enum.find(duplicate_groups, fn group ->
  ...
  describe "legislation validation and normalization" do
    test "Ash resource validates and normalizes titles on creation" do
      ...
      attrs = %{
        legislation_title: "VALIDATION TEST ACT",  # All caps
    ...
    test "Ash resource validates positive number" do
      attrs = %{
        ...
        legislation_year: 2023,
        legislation_number: -1,  # Negative number
        legislation_type: :act
      }
