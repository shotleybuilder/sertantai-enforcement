defmodule EhsEnforcement.Enforcement.DomainTest do
  ...
  describe "domain function interfaces" do
    test "update_case_from_scraping/3 function interface", %{agency: agency, offender: offender} do
      ...
      assert {:ok, updated_case} = Enforcement.update_case_from_scraping(
        case_record, 
        scraping_params
      )
    ...
    test "sync_case_from_airtable/3 function interface", %{agency: agency, offender: offender} do
      ...
      # Test the domain function directly
      assert {:ok, synced_case} = Enforcement.sync_case_from_airtable(
        case_record, 
        airtable_params
      )
    ...
    test "functions accept actor parameter for authorization", %{agency: agency, offender: offender} do
      {:ok, case_record} = Enforcement.create_case(%{
        agency_id: agency.id,
        offender_id: offender.id,
        regulator_id: "HSE003"
  ...
  describe "Ash forms for scraping and syncing" do
    test "form for scraping updates" do
      form = AshPhoenix.Form.for_action(
      ...
      assert AshPhoenix.Form.value(validated_form, :url) == "https://hse.gov.uk/prosecutions/case/123"
    end
    ...
    test "forms reject invalid field values" do
      scraping_form = AshPhoenix.Form.for_action(
        EhsEnforcement.Enforcement.Case, 
        :update_from_scraping,
        forms: [auto?: false]
      )
  ...
  describe "domain helper functions" do
    test "change_case_for_scraping/2 creates correct changeset" do
      ...
      changeset = Enforcement.change_case_for_scraping(
        %EhsEnforcement.Enforcement.Case{}, 
        case_params
      )
    ...
    test "change_case_for_airtable_sync/2 creates correct changeset" do
      ...
      changeset = Enforcement.change_case_for_airtable_sync(
        %EhsEnforcement.Enforcement.Case{}, 
        case_params
      )
  ...
  describe "backwards compatibility" do
    test "legacy sync_case function still works but uses new action", %{agency: agency, offender: offender} do
      {:ok, case_record} = Enforcement.create_case(%{
        agency_id: agency.id,
        offender_id: offender.id,
