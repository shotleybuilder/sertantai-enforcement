defmodule EhsEnforcement.Enforcement.WorkflowsIntegrationTest do
  ...
  describe "scraping workflow (HSE website → Postgres)" do
    ...
    test "scraping workflow handles multiple updates correctly", %{test_case: test_case} do
      ...
      {:ok, updated_case_1} =
        Enforcement.update_case_from_scraping(test_case, %{
          offence_result: "Guilty"
        })

      # Second scraping update
      {:ok, updated_case_2} =
        Enforcement.update_case_from_scraping(updated_case_1, %{
  ...
  describe "syncing workflow (Airtable → Postgres)" do
    ...
    test "syncing workflow updates timestamps on every sync", %{test_case: test_case} do
      # First sync
      {:ok, synced_case_1} =
        Enforcement.sync_case_from_airtable(test_case, %{
          offence_result: "Not guilty"
        })
      ...
      # Second sync
      {:ok, synced_case_2} =
        Enforcement.sync_case_from_airtable(synced_case_1, %{
  ...
  describe "workflow separation and independence" do
    test "scraping and syncing workflows can be used independently", %{test_case: test_case} do
      ...
      # Preserved from scraping
      assert synced_case.url == "https://hse.gov.uk/prosecutions/case/workflow_001"
      # Added from sync
      assert Decimal.equal?(synced_case.offence_fine, Decimal.new("12000.00"))
      # Sync timestamp set
      assert synced_case.last_synced_at != nil
    ...
    test "workflows can happen in any order without interference", %{test_case: test_case} do
      ...
      {:ok, scraped_case} =
        Enforcement.update_case_from_scraping(synced_case, %{
      ...
      # Preserved from sync
      assert scraped_case.offence_result == "Not guilty"
      # Preserved from sync
      assert Decimal.equal?(scraped_case.offence_fine, Decimal.new("0.00"))
      # Added from scraping
      assert Decimal.equal?(scraped_case.offence_costs, Decimal.new("1500.00"))
      # Added from scraping
      assert scraped_case.url == "https://hse.gov.uk/prosecutions/case/reverse_001"
      # Unchanged by scraping
      assert scraped_case.last_synced_at == sync_timestamp
    end
  ...
  describe "real-world workflow scenarios" do
    test "typical scraping session followed by Airtable migration", %{test_case: test_case} do
      ...
      {:ok, migrated_case} =
        Enforcement.sync_case_from_airtable(scraped_case, %{
          ...
          # Confirmed
          offence_fine: Decimal.new("25000.00"),
          # Slightly different (official record)
          offence_costs: Decimal.new("4800.00"),
      ...
      # Airtable wins
      assert Decimal.equal?(migrated_case.offence_costs, Decimal.new("4800.00"))
      # Preserved from scraping
      assert migrated_case.url == "https://hse.gov.uk/prosecutions/case/real_world_001"
      # Added from Airtable
      assert migrated_case.offence_breaches != nil
      # Migration timestamp
      assert migrated_case.last_synced_at != nil
    end
    ...
    test "handles concurrent updates gracefully", %{test_case: test_case} do
      ...
      # From first scraping
      assert final_case.offence_result == "Guilty"
      # From first scraping
      assert final_case.url == "https://hse.gov.uk/prosecutions/case/concurrent_001"
      # From Airtable sync
      assert Decimal.equal?(final_case.offence_fine, Decimal.new("7500.00"))
      # From Airtable sync
      assert final_case.offence_breaches == "Multiple violations"
      # From second scraping
      assert Decimal.equal?(final_case.offence_costs, Decimal.new("2000.00"))
      # From second scraping
      assert final_case.related_cases == "Related to case HSE_002"
