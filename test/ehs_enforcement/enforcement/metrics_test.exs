defmodule EhsEnforcement.Enforcement.MetricsTest do
  ...
  setup do
    ...
    recent_date = Date.add(Date.utc_today(), -5)  # 5 days ago (within all periods)
    old_date = Date.add(Date.utc_today(), -400)   # 400 days ago (outside all periods)
    ...
    # EA cases
    {:ok, _case3} = Enforcement.create_case(%{
    ...
    {:ok, _old_case} = Enforcement.create_case(%{
      agency_id: hse_agency.id,
      offender_id: offender1.id,
      regulator_id: "HSE003",
      offence_result: "Guilty",
  ...
  describe "refresh_all_metrics/1" do
    test "generates Tier 1 + Tier 2 metrics (9 rows with 2 agencies)" do
      ...
      combinations = Enum.map(results, fn m ->
    ...
    test "Tier 1 metrics aggregate all agencies correctly", %{hse_agency: hse, ea_agency: ea} do
      ...
      week_all = Enum.find(results, fn m ->
    ...
    test "Tier 2 metrics filter by agency_id correctly", %{hse_agency: hse, ea_agency: ea} do
      ...
      week_hse = Enum.find(results, fn m ->
      ...
      week_ea = Enum.find(results, fn m ->
    ...
    test "uses SQL aggregations (zero record loading)" do
      ...
      recent_cases = Enforcement.list_cases_with_filters!([])
      |> Enum.filter(fn c ->
