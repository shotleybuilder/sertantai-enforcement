defmodule EhsEnforcementWeb.Admin.NoticeLive.EaNoticeProgressTest do
  ...
  describe "ScrapeSession Field Preservation" do
    ...
    test "multiple single-field updates show the problem", %{admin_user: admin_user} do
      ...
      assert session.cases_found == 41,
             "After update 1, cases_found should still be 41, got #{session.cases_found}"
      ...
      assert session.cases_found == 41,
             "After update 2, cases_found should still be 41, got #{session.cases_found}"

      assert session.cases_created == 5,
             "After update 2, cases_created should still be 5, got #{session.cases_created}"
      ...
      assert session.cases_found == 41,
             "After update 3, cases_found should still be 41, got #{session.cases_found}"

      assert session.cases_created == 5,
             "After update 3, cases_created should still be 5, got #{session.cases_created}"

      assert session.cases_updated == 2,
             "After update 3, cases_updated should still be 2, got #{session.cases_updated}"
      ...
      assert session.cases_found == 41,
             "After completion, cases_found should be 41, got #{session.cases_found}"

      assert session.cases_created == 5,
             "After completion, cases_created should be 5, got #{session.cases_created}"
