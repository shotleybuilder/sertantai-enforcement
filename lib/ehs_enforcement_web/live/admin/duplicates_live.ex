defmodule EhsEnforcementWeb.Admin.DuplicatesLive do
  ...
  def handle_event("toggle_record", %{"id" => record_id}, socket) do
    ...
    new_selected = if MapSet.member?(selected, record_id) do
  ...
  defp load_active_tab_duplicates(socket) do
    ...
    try do
      ...
      :exit, {:timeout, _} ->
        socket
        |> assign(:loading, false)
        |> put_flash(:error, "Duplicate detection timed out. Please try again with a smaller dataset.")
    end
  end
  ...
  defp execute_delete_action(socket, record_ids) do
    ...
    results = Enum.map(record_ids, fn record_id ->
  ...
  defp execute_merge_action(socket, _record_ids) do
    ...
    {:noreply,
     socket
     |> assign(:action_confirmation, nil)
     |> put_flash(:info, "Merge functionality coming soon - please manually review and delete unwanted records")}
  end
  ...
  defp find_duplicate_offenders(current_user) do
    try do
      # Use a more efficient query with limit and timeout
      query = Offender |> Ash.Query.limit(500)  # Smaller limit for offenders
  ...
  defp remove_overlapping_groups(groups) do
    ...
    record_to_group = groups_with_index
