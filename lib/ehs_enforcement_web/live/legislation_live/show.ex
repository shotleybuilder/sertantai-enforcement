defmodule EhsEnforcementWeb.LegislationLive.Show do
    ...
    try do
      ...
      case Enforcement.get_legislation(id, actor: socket.assigns.current_user) do
        {:ok, legislation} ->
          ...
          {related_cases, related_notices} = load_related_data(legislation.id, socket.assigns.cases_page, socket.assigns.notices_page, socket.assigns.page_size)
  ...
  def handle_event("paginate_cases", %{"page" => page_str}, socket) do
    case Integer.parse(page_str) do
      {page, _} when page > 0 ->
        {related_cases, _related_notices} = load_related_data(socket.assigns.legislation.id, page, socket.assigns.notices_page, socket.assigns.page_size)
  ...
  def handle_event("paginate_notices", %{"page" => page_str}, socket) do
    case Integer.parse(page_str) do
      {page, _} when page > 0 ->
        {_related_cases, related_notices} = load_related_data(socket.assigns.legislation.id, socket.assigns.cases_page, page, socket.assigns.page_size)
  ...
  def handle_event("export_legislation", _params, socket) do
    case socket.assigns.legislation do
      ...
      legislation_record ->
        csv_data = generate_legislation_csv(legislation_record, socket.assigns.related_cases, socket.assigns.related_notices)
    ...
    try do
      ...
      case offences_result do
        {:ok, offences} ->
          ...
          cases = if Enum.empty?(case_ids) do
            []
          else
            case_offset = (cases_page - 1) * page_size
            case_query = Enforcement.Case
          ...
          notices = if Enum.empty?(notice_ids) do
            []
          else
            notice_offset = (notices_page - 1) * page_size
            notice_query = Enforcement.Notice
