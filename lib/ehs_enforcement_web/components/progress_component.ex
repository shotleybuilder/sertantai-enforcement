defmodule EhsEnforcementWeb.Components.ProgressComponent do
  ...
  defp ea_progress_percentage(progress) do
    case progress.status do
      ...
      :running ->
        ...
        processed = get_field(progress, :processed) ||
  ...
  defp render_common_statistics(assigns) do
    ...
    item_label = if Map.has_key?(assigns.progress, :notices_found) or Map.has_key?(assigns.progress, :notices_created) do
      "Notices"
    else
      "Cases"
    end

    assigns = assign(assigns, found: found, created: created, updated: updated, exist_total: exist_total, item_label: item_label)
