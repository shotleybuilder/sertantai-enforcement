defmodule Mix.Tasks.PopulateOffenderAgencies do
  ...
  defp populate_agencies(opts) do
    ...
    case Ash.read(offenders_query) do
      {:ok, offenders} ->
        ...
        Logger.info(
          "📊 Found #{empty_count} offenders with empty agencies (out of #{total_count} total)"
        )
  ...
  defp get_offender_agencies(offender_id) do
    ...
    case_agencies =
      case Ash.read(EhsEnforcement.Enforcement.Case) do
        {:ok, cases} ->
          cases
  ...
  defp verify_results do
    ...
    case Ash.read(EhsEnforcement.Enforcement.Offender) do
      {:ok, all_offenders} ->
        ...
        Logger.info(
          "📈 Final stats: #{with_agencies}/#{total} offenders (#{percentage}%) have agencies"
        )
