defmodule EhsEnforcement.Scraping.Strategies.HSE.NoticeStrategy do
  @moduledoc """
  Strategy for scraping HSE enforcement notices from the HSE notices database.

  This strategy implements the `ScrapeStrategy` behavior for HSE notice scraping,
  which uses page-based pagination to fetch enforcement notice data from the HSE website.

  ## Scraping Pattern

  HSE notice scraping follows a page-based pattern similar to case scraping:
  - Starts at `start_page` (default: 1)
  - Scrapes up to `max_pages` pages
  - Each page contains multiple notices
  ...
  - `"notices"` - Enforcement notices (default for this strategy)
  - `"convictions"` - Court cases (use CaseStrategy instead)
  - `"appeals"` - Appeal cases

  ## Example Usage

      iex> strategy = EhsEnforcement.Scraping.Strategies.HSE.NoticeStrategy
      iex> params = %{start_page: 1, max_pages: 10, database: "notices"}
  ...
  def scrape_data(params) do
    # Delegate to existing HSE.NoticeScraper
    # The scraper expects keyword list with page_number and country
    page_number = params[:page_number] || params.start_page
    country = params[:country] || params.country || @default_country

    # NoticeScraper.get_hse_notices returns the notices directly or {:error, reason}
    case NoticeScraper.get_hse_notices(page_number: page_number, country: country) do
      {:error, reason} -> {:error, reason}
      notices when is_list(notices) -> {:ok, notices}
      result -> {:ok, List.wrap(result)}
    end
  end

  @impl true
  def process_record(notice_data, _session) do
    # Delegate to existing HSE.NoticeProcessor
  ...
  def format_progress_display(%ScrapeSession{} = session) do
    ...
    # This is a legacy naming convention that will be addressed in future refactors
    %{
  ...
  @impl true
  def strategy_name, do: "HSE Notice Scraping"
  ...
  defp validate_country(nil), do: {:ok, @default_country}

  defp validate_country(country) when is_binary(country) and byte_size(country) > 0 do
    {:ok, country}
  end

  defp validate_country(_), do: {:error, "country must be a non-empty string"}
end
