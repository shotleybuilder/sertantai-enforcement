defmodule EhsEnforcement.Scraping.Hse.CaseScraper do
  ...
  def cases_exist?(regulator_ids) when is_list(regulator_ids) do
    try do
      results = Enum.reduce(regulator_ids, %{}, fn regulator_id, acc ->
        case case_exists?(regulator_id) do
          {:ok, exists?} -> Map.put(acc, regulator_id, exists?)
          {:error, _} -> Map.put(acc, regulator_id, false) # Assume doesn't exist on error
        end
      end)
    ...
    try do
      enriched = Enum.map(cases, fn case_data ->
        case fetch_case_details(case_data.regulator_id, database, opts) do
          ...
          {:error, _} -> case_data # Return base case if details fail
        end
      end)
  ...
  defp extract_case_details_from_rows(rows, database) do
    Enum.reduce(rows, %{}, fn
      ...
      [
        ...
        breach_data = case fetch_breach_details(String.trim_leading(breach_url, "../"), database) do
          {:ok, data} -> data
          {:error, _} -> %{}
        end
        
        related_data = case fetch_related_cases(String.trim_leading(related_url, "../"), database) do
      ...
      [
        ...
        breaches_data = case fetch_breaches_details(String.trim_leading(breaches_url, "../"), database) do
          {:ok, data} -> data
          {:error, _} -> %{}
        end
  ...
  defp fetch_breach_details("breach/breach_details.asp?SF=BID&SV=" <> case_number, database) do
    url = "search/search.asp?ST=B&SN=F&EO=%3D&SF=CN&SV=" <> Regex.replace(~r/\d{3}$/, case_number, "")
    fetch_breaches_details(url, database)
  end
  
  defp fetch_breaches_details("search/search.asp?ST=B&SN=F&EO=%3D&SF=CN&SV=" <> case_number, database) do
    base_url = String.replace(@base_url_template, "%{database}", database) 
  ...
  defp parse_breach_list(html) do
    try do
      ...
      breach_data = Enum.reduce(rows, %{offence_number: 0, offence_breaches: []}, fn
        [
          {"td", _, _},
          {"td", _, _},
      ...
      breach_data = Map.put(breach_data, :offence_breaches, Enum.join(breach_data.offence_breaches, "; "))
  ...
  defp fetch_related_cases("search/search.asp?ST=C&SN=R&EO=%3D&SF=RCN&SV=" <> case_number, database) do
  ...
  defp parse_related_cases_list(html) do
    try do
      ...
      related_cases = Enum.reduce(rows, [], fn
        [
