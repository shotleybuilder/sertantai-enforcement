defmodule EhsEnforcement.Scraping.Ea.CaseScraper do
  ...
  @action_types [
    ...
    {:caution,
     "http://environment.data.gov.uk/public-register/enforcement-action/def/action-type/caution"},
    {:enforcement_notice,
     "http://environment.data.gov.uk/public-register/enforcement-action/def/action-type/enforcement-notice"}
  ]
  # Default 3s between detail page requests
  @default_detail_delay_ms 3000
  @max_retries 3
  ...
  defmodule EaSummaryRecord do
    ...
    defstruct [
      :ea_record_id,
      :offender_name,
      # Address from summary table (may be incomplete)
      :summary_address,
      :action_date,
      :action_type,
      :detail_url,
  ...
  def scrape_enforcement_actions(date_from, date_to, action_types, opts)
    ...
    case collect_all_summary_records(
           date_from,
           date_to,
           action_types,
           page_start,
  ...
  def scrape_enforcement_actions(date_from, date_to, action_type, opts)
      when is_atom(action_type) do
  ...
  defp collect_all_summary_records(date_from, date_to, action_types, page_start, max_pages, opts) do
    ...
    results =
      Enum.reduce_while(action_types, {:ok, []}, fn action_type, {:ok, acc} ->
        ...
        case collect_summary_records_for_action_type(
               date_from,
               date_to,
               action_type,
               page_start,
          ...
          {:error, reason} ->
            Logger.error(
              "Failed to collect records for action type #{action_type}: #{inspect(reason)}"
            )
  ...
  defp collect_summary_records_for_action_type(
         date_from,
         date_to,
         action_type,
         _page_start,
  ...
  defp scrape_single_action_type(date_from, date_to, action_type, opts) do
    ...
    case scrape_single_summary_page(date_from, date_to, action_type, opts) do
      {:ok, records} ->
        Logger.info(
          "Action type #{action_type}: #{length(records)} total records (complete result set)"
        )
  ...
  defp collect_all_detail_records(summary_records, opts) do
    Logger.debug(
      "Stage 2: Collecting detail records for #{length(summary_records)} summary records"
    )

    results =
      Enum.reduce_while(summary_records, {:ok, []}, fn summary_record, {:ok, acc} ->
        case fetch_detail_record(summary_record, opts) do
          ...
          {:error, reason} ->
            Logger.warning(
              "Failed to fetch detail for record #{summary_record.ea_record_id}: #{inspect(reason)}"
            )
    ...
    case results do
      {:ok, detail_records} ->
        ...
        {:ok, Enum.reverse(detail_records)}
  ...
  defp build_search_url(action_type, date_from, date_to, opts) do
    ...
    params = %{
      # Always include empty name-search as in working URL
      "name-search" => "",
      "actionType" => action_type_url,
      # Include empty offenceType
      "offenceType" => "",
      # Include empty agencyFunction
      "agencyFunction" => "",
      "after" => Date.to_string(date_from),
      # Use date_to parameter for proper filtering
      "before" => Date.to_string(date_to)
    }

    # Override name search if specifically provided
    params =
      case Keyword.get(opts, :name_search) do
        nil -> params
        name when name != "" -> Map.put(params, "name-search", name)
        # Keep empty if empty string provided
        _ -> params
      end
    ...
    try do
      ...
      case cells do
        [name_cell, address_cell, date_cell | _] ->
          ...
          detail_url =
            case Floki.find(name_cell, "a") do
              [{"a", attrs, _}] ->
          ...
          if offender_name != "" && action_date && detail_url && ea_record_id do
            ...
            Logger.debug(
              "Insufficient data for EA record: name='#{offender_name}', date=#{inspect(action_date)}, url=#{detail_url}, id=#{ea_record_id}"
            )
        ...
        [name_cell, date_cell | _] ->
          ...
          if offender_name != "" && action_date && detail_url && ea_record_id do
            ...
            Logger.debug(
              "Insufficient data for EA record (2-col): name='#{offender_name}', date=#{inspect(action_date)}, url=#{detail_url}, id=#{ea_record_id}"
            )
  ...
  defp extract_legal_reference(document) do
    ...
    case {act, section} do
      ...
      {act_val, _} when is_binary(act_val) ->
        String.trim(act_val)
  ...
  if Mix.env() == :test do
    ...
    def test_extract_summary_record_from_row(row, action_type, timestamp),
      do: extract_summary_record_from_row(row, action_type, timestamp)
  end
end
