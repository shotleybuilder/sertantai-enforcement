defmodule EhsEnforcement.Scraping.Ea.NoticeScraper do
  ...
  def collect_summary_records(date_from, date_to, opts \\ []) do
    ...
    case CaseScraper.collect_summary_records_for_action_type(
           date_from, 
           date_to, 
           :enforcement_notice,  # Fixed action type for notices
           opts
         ) do
      {:ok, summary_records} ->
        Logger.info("EA NoticeScraper: Found #{length(summary_records)} enforcement notice summaries")
        {:ok, summary_records}
  ...
  def fetch_detail_record(summary_record, opts \\ []) do
    Logger.debug("EA NoticeScraper: Fetching detail record for notice: #{summary_record.ea_record_id}")
    
    case CaseScraper.fetch_detail_record_individual(summary_record, opts) do
      {:ok, detail_record} ->
        Logger.debug("EA NoticeScraper: Successfully fetched notice details for: #{detail_record.ea_record_id}")
        {:ok, detail_record}
        
      {:error, reason} ->
        Logger.warning("EA NoticeScraper: Failed to fetch notice details for #{summary_record.ea_record_id}: #{inspect(reason)}")
        {:error, reason}
    end
  end
  ...
  def collect_and_enrich_notices(date_from, date_to, opts \\ []) do
    ...
    with {:ok, summary_records} <- collect_summary_records(date_from, date_to, opts),
      ...
      Logger.info("EA NoticeScraper: Successfully enriched #{length(enriched_records)} notice records")
      {:ok, enriched_records}
    else
      {:error, reason} ->
  ...
  defp enrich_notice_batch(summary_records, opts) do
    ...
    enriched_records = Enum.reduce(summary_records, [], fn summary, acc ->
      ...
      case fetch_detail_record(summary, opts) do
        {:ok, detail_record} ->
          ...
          if enforcement_notice?(detail_record) do
            ...
            Logger.warning("EA NoticeScraper: Skipping non-notice record: #{detail_record.ea_record_id}")
            acc
          end
          
        {:error, reason} ->
          Logger.warning("EA NoticeScraper: Failed to enrich #{summary.ea_record_id}: #{inspect(reason)}")
          acc
      end
    end)
