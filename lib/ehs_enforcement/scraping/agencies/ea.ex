defmodule EhsEnforcement.Scraping.Agencies.Ea do
  ...
  def validate_params(opts) do
    ...
    validated_params = %{
      ...
      start_page: 1,
      # EA processes everything in one batch
      max_pages: 1,
  ...
  def start_scraping(validated_params, _config) do
    ...
    ash_session_params = %{
      ...
      # EA-specific database identifier
  ...
  defp load_scraping_config(opts) do
    ...
    case ScrapingConfig.get_active_config(opts) do
      ...
      {:error, reason} ->
        Logger.error(
          "EA: Failed to load scraping configuration: #{inspect(reason)}, using fallback values"
        )
  ...
  defp validate_action_types(action_types) do
    ...
    if invalid_types == [] do
      ...
      {:error,
       "Invalid action types: #{inspect(invalid_types)}. Valid types: #{inspect(valid_types)}"}
    end
  end
  ...
  defp process_ea_action_types_individually(session, action_types, validated_params) do
    ...
    final_session =
      Enum.reduce(action_types, session, fn action_type, current_session ->
        Logger.info("EA: Processing action type: #{action_type}")

        case process_single_action_type_individually(
               current_session,
               action_type,
               validated_params
             ) do
  ...
  defp process_filtered_records_individually(
      ...
      Enum.reduce(filtered_records, initial_state, fn summary_record, acc ->
        ...
        case CaseScraper.fetch_detail_record_individual(summary_record,
          ...
          {:ok, detail_record} ->
            ...
            record_result =
              process_and_save_single_ea_record(
                acc.current_session,
                detail_record,
                action_type,
                actor
            ...
            updated_session =
              update_session_with_single_record_progress(acc.current_session, record_result)
          ...
          {:error, reason} ->
            Logger.warning(
              "EA: Failed to fetch detail for #{summary_record.ea_record_id}: #{inspect(reason)}"
            )
    ...
    create_ea_action_type_processing_log(
      session,
      action_type,
      final_results.processed_cases,
      final_results
    )
  ...
  defp merge_record_result(acc, record_result) do
    case record_result.status do
      ...
      :updated ->
        %{
          acc
          | # Count updates as "created" for UI purposes
            cases_created: acc.cases_created + 1,
            processed_cases: [record_result.transformed_data | acc.processed_cases]
        }
  ...
  defp update_session_with_single_record_progress(session, record_result) do
    ...
    case Ash.update(session, update_params) do
      {:ok, updated_session} ->
        Logger.debug(
          "EA: Updated session progress for real-time feedback (#{record_result.record_type})"
        )
  ...
  defp determine_final_status(session) do
    cond do
      ...
      session.cases_found > 0 && session.cases_processed == 0 ->
        Logger.info(
          "EA: Session stopped before processing any cases (found: #{session.cases_found})"
        )
      ...
      session.cases_found > 0 && session.cases_processed < session.cases_found ->
        Logger.info(
          "EA: Session stopped after processing #{session.cases_processed}/#{session.cases_found} cases"
        )
  ...
  defp create_ea_action_type_processing_log(session, action_type, processed_cases, results) do
    ...
    case_summary =
      Enum.map(processed_cases, fn case_data ->
        %{
    ...
    log_params = %{
      session_id: session.session_id,
      agency: :ea,
      # EA batch number (integer, not string)
      batch_or_page: 1,
    ...
    case Ash.create(ProcessingLog, log_params) do
      {:ok, _log} ->
        Logger.debug(
          "EA: Created unified processing log for #{action_type} in session #{session.session_id}"
        )

      {:error, reason} ->
        Logger.warning(
          "EA: Failed to create unified processing log for #{action_type}: #{inspect(reason)}"
        )
    end
  end
