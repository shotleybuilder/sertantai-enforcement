defmodule EhsEnforcement.Enforcement.Metrics do
  ...
  actions do
    ...
    create :refresh do
      ...
      accept [
        ...
        :period, :period_label, :days_ago, :cutoff_date,

        # Filter dimensions
        :agency_id, :record_type, :offender_id, :legislation_id,

        # Aggregate counts
        :recent_cases_count, :recent_notices_count,
        :total_cases_count, :total_notices_count, :total_offences_count,

        # Financial aggregates
        :total_fines_amount, :total_costs_amount,

        # Agency and breakdown data
        :active_agencies_count, :agency_stats,
        :offender_breakdown, :legislation_breakdown,
  ...
  def refresh_all_metrics(calculated_by \\ :admin) do
    ...
    tier1_results = for period <- [:week, :month, :year] do
    ...
    tier2_results = for period <- [:week, :month, :year],
                        agency <- agencies do
  ...
  defp refresh_metric_combination(opts) do
    ...
    agency_id_binary = if agency_id && is_binary(agency_id) && byte_size(agency_id) > 16 do
    ...
    {days_ago, period_label} = case period do
      :week -> {7, "Last 7 Days"}
    ...
    Logger.debug("Refreshing metric: period=#{period}, agency_id=#{inspect(agency_id)}, record_type=#{inspect(record_type)}")
    ...
    agency_stats = if is_nil(agency_id) do
    ...
    recent_activity = fetch_recent_activity_json(cutoff_date, agency_id_binary, record_type, limit: 100)
    ...
    attrs = %{
      ...
      offender_id: nil,      # Future use
      legislation_id: nil,   # Future use
      ...
      offender_breakdown: %{},      # TODO: Implement in future phase
      legislation_breakdown: %{},   # TODO: Implement in future phase
  ...
  defp get_recent_stats(cutoff_date, agency_id, record_type) do
    ...
    record_type_filter = case record_type do
      :case -> "AND type = 'case'"
    ...
    [recent_cases_count, recent_notices_count, total_fines, total_costs] = result.rows |> List.first()
  ...
  defp calculate_agency_breakdown(cutoff_date, record_type) do
    ...
    stats_by_agency_id = result.rows
    ...
    total_notices = stats_by_agency_id |> Map.values() |> Enum.map(& &1.notice_count) |> Enum.sum()
    total_actions = total_cases + total_notices

    # Merge with agency details and calculate percentages
    agency_stats = all_agencies
    |> Enum.map(fn agency ->
      stats = Map.get(stats_by_agency_id, agency.id, %{case_count: 0, notice_count: 0, total_fines: Decimal.new(0)})
      ...
      %{
        ...
        case_percentage: if(total_cases > 0, do: Float.round(case_count / total_cases * 100, 1), else: 0.0),
        action_percentage: if(total_actions > 0, do: Float.round(action_count / total_actions * 100, 1), else: 0.0)
      }
    end)
  ...
  defp fetch_recent_activity_json(cutoff_date, agency_id, record_type, opts \\ []) do
    ...
    result.rows
    |> Enum.map(fn [id, record_type, display_type, regulator_id, action_date, agency_id, offender_id, fine_amount, costs_amount, organization, description, url] ->
      # Parse date string to Date struct for Calendar.strftime compatibility
      parsed_date = case action_date do
        %Date{} = date -> date
