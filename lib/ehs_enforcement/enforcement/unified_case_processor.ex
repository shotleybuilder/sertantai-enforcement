defmodule EhsEnforcement.Enforcement.UnifiedCaseProcessor do
  ...
  defp handle_duplicate_case(case_attrs, original_error, actor) do
    ...
    case Enforcement.Case 
      ...
      {:ok, existing_case} when not is_nil(existing_case) ->
        ...
        if needs_update?(existing_case, update_attrs) do
          ...
          case Enforcement.update_case_from_scraping(existing_case, update_attrs, update_opts) do
            ...
            {:error, update_error} ->
              Logger.error("Failed to update existing case #{case_attrs.regulator_id}: #{inspect(update_error)}")
              {:error, original_error}
          end
        else
          # For cases with identical data, manually trigger PubSub event without updating record
          Logger.debug("Case already exists with identical data, manually triggering PubSub for UI: #{existing_case.regulator_id}")
          ...
          notification = %Ash.Notifier.Notification{
            ...
            data: existing_case,
            changeset: nil,
            metadata: %{processing_status: :existing}  # ← Pass the real DB result
          }
          ...
          Logger.info("Successfully triggered PubSub for existing case (UI): #{existing_case.regulator_id}")
      ...
      {:error, query_error} ->
        Logger.error("Failed to query existing case #{case_attrs.regulator_id}: #{inspect(query_error)}")
        {:error, original_error}
    end
  end
  ...
  defp needs_update?(existing_case, update_attrs) do
    Enum.any?(update_attrs, fn {field, new_value} ->
      existing_value = case field do
        :url -> existing_case.regulator_url  # Handle field name mapping
