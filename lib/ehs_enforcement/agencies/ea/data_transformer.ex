defmodule EhsEnforcement.Agencies.Ea.DataTransformer do
  ...
  def transform_ea_record(%EaDetailRecord{} = ea_record) do
    ...
    offence_unique_hash = generate_offence_unique_hash(
    ...
    %{
      ...
      # Enforcement details
      action_date: ea_record.action_date,  # Already parsed by CaseScraper
      action_type: ea_record.action_type,  # Already normalized
      ...
      legal_reference: ea_record.legal_reference || build_legal_reference(ea_record.act, ea_record.section),
  ...
  defp generate_offence_unique_hash(offence_description, act, section) do
    ...
    content = "#{String.trim(offence_description)}|#{String.trim(act || "")}|#{String.trim(section || "")}"
  ...
  defp normalize_action_type(action_type_url) do
    cond do
      is_binary(action_type_url) and String.contains?(action_type_url, "court-case") -> :court_case
      is_binary(action_type_url) and String.contains?(action_type_url, "caution") -> :caution  
      is_binary(action_type_url) and String.contains?(action_type_url, "enforcement-notice") -> :enforcement_notice
      is_atom(action_type_url) -> action_type_url
      true -> :unknown
    end
  end
  ...
  defp generate_regulator_id_from_detail(%EaDetailRecord{} = ea_record) do
    ...
    date_part = case ea_record.action_date do
      %Date{} = date -> date |> Date.to_string() |> String.replace("-", "")
      _ -> "00000000"
    end
    
    action_code = case ea_record.action_type do
      :court_case -> "CC"
      :caution -> "CA"
  ...
  defp generate_regulator_id(data) do
    ...
    action_code = case data.action_type do
      :court_case -> "CC"
      :caution -> "CA"
      :enforcement_notice -> "EN"
      _ -> "XX"
