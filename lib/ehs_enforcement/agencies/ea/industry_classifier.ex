defmodule EhsEnforcement.Agencies.Ea.IndustryClassifier do
  ...
  defp classify_by_patterns(ea_sector_name, opts) do
    ...
    best_match =
      ...
      |> Enum.map(fn {pattern, category, base_confidence} ->
        if Regex.match?(pattern, ea_sector_name) do
          ...
          adjusted_confidence =
            calculate_adjusted_confidence(ea_sector_name, pattern, base_confidence)
    ...
    case best_match do
      ...
      {_pattern, category, confidence} ->
        ...
        Logger.warning(
          "Low confidence industry classification: '#{ea_sector_name}' → #{category} (#{confidence})"
        )
  ...
  defp calculate_adjusted_confidence(ea_sector_name, _pattern, base_confidence) do
    ...
    length_multiplier =
      case sector_length do
        # Very detailed sector names
        len when len > 40 -> 1.1
        # Detailed sector names
        len when len > 25 -> 1.05
        # Standard sector names
        len when len > 15 -> 1.0
        # Short/generic sector names
        _ -> 0.95
      end
  ...
  defp generate_accuracy_recommendations(mappings) do
    ...
    recommendations =
      if length(low_confidence_sectors) > 0 do
        [
          "Consider manual review for #{length(low_confidence_sectors)} low-confidence sectors"
          | recommendations
        ]
      else
        recommendations
    ...
    recommendations =
      if Enum.any?(mappings, &(&1.method == :no_match)) do
